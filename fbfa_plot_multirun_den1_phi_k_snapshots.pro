...
for id=0,ndims_all-1 do begin
   ...
   for ip=0,n_paths-1 do begin
      ...
      savename = 'phi'+savebase
      savepath = expand_path(path)+path_sep()+savename
      restore, filename=savepath,/verbose
      ;; print, time
      phi_spectrum = spectrum[*,it]*scale
      sys_tf = systime(1)
      ...
      ifk1 = find_closest(k_vals,fk1)
      fitx = alog10(k_vals[ifk0:ifk1])
      phi_fity = alog10((reverse(phi_spectrum))[ifk0:ifk1])
      phi_fitc = linfit(fitx,phi_fity,yfit=phi_yfit, $
                           chisqr=phi_chisqr,prob=phi_prob,sigma=phi_sigma)
      help, phi_chisqr,phi_prob
      ...
      if where(strmatch(plot_fits, 'phi')) ge 0 then begin
         opl = plot(10^fitx, $
                    10^phi_yfit, $
                    color = 'red', $
                    thick = 2, $
                    /overplot)
         slope_str = strcompress(string(phi_fitc[1], $
                                        format='(f6.1)'),/remove_all)
         txt = text(current_pos[2]-0.12, $
                    current_pos[3]-0.15, $
                    slope_str, $
...
frmpath = get_base_dir()+path_sep()+ $
          ...
          'np_sqr-k_spectrum-multirun-snapshots.pdf'
