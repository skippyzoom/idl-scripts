;+
; Flow angle project: Similar to fbfa_build_perp_cfft_t.pro except that 
; this routine creates the full 3-D FFT. The 2-D section is
; identical. It turns out that Building and saving the full 3-D FFT
; takes about an hour and 45 minutes whereas restoring it takes about
; 6 minutes.
;
...
if n_files_sub eq nt then begin
   ...
   if params.ndim_space eq 2 then begin
      ...
      ;;==Allocate the array
      fftdata = complexarr(nx,ny,nz,(itf-it0)/itd+1)
      ...
      ;;==Save the data to disk
      savename = dataname+'-cfft-full.sav'
      savepath = expand_path(path)+path_sep()+savename
      sys_t0 = systime(1)
      save, time,fftdata,filename=savepath
      sys_tf = systime(1)
      print, "Elapsed minutes for save: ",(sys_tf-sys_t0)/60.
