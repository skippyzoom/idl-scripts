...
;;==Compute centroids
t0 = systime(1)
;; fdata = abs(den1fft_t[nx/2,*,*,*])
fdata = mean(abs(den1fft_t[i_x0:i_xf-1,*,*,*]),dim=1)
fdata = reform(fdata)
tmp_theta = fltarr(sub_nt)
tmp_kmag = fltarr(sub_nt)
for it=0,sub_nt-1 do begin
   cm = calc_fft_centroid(rotate(fdata[*,*,it],1), $
                          xrange = [c_z0,c_zf-1], $
                          yrange = [c_y0,c_yf-1], $
                          /zero_dc, $
                          mask_threshold = 1e-1, $
   ...
   dev_zcm = cm.dev_x
   dev_ycm = cm.dev_y
   tmp_kmag[it] = sqrt((dkz*zcm)^2 + (dky*ycm)^2)
   tmp_theta[it] = atan(ycm,zcm)
endfor
tf = systime(1)
...
;;==Create frame
imgf = fdata[i_z0:i_zf-1,i_y0:i_yf-1]
;; imgx = kzdata[i_z0:i_zf-1]
;; imgy = kydata[i_y0:i_yf-1]
imgx = kzdata[i_z0-1:i_zf-2]
imgy = kydata[i_y0-2:i_yf-3]
;; imgx = indgen(i_zf-i_z0)-(i_zf-i_z0)/2
