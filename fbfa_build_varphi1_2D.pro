...
t0 = systime(1)
varphi1 = complexarr(nx,ny,sub_nt)
tf = systime(1)
...
for it=0,sub_nt-1 do begin
   n1 = transpose(get_h5_data(sub_files[it],'den1'),[1,0])
   T1 = temp1[*,*,it]
...
;;==Save array
t0 = systime(1)
save, varphi1,filename=expand_path(path)+path_sep()+'varphi1-2D.sav'
tf = systime(1)
